...
theme_MB = theme(
    ...
    strip.text = element_text(size = rel(0.72)), #margin = margin(t = 1, b = 1)
...
panel_map <- c(
  "ST-CHE" = "Chemistry",
  "ST-ECO" = "Economics",
  "ST-ENG" = "Information & Engineering",
...
ggsave("Output/Fig_1ab.png",gg, width = 18, height = 5.5, units = 'cm')


grid.arrange(
          grobs = list(p_gr, p_grB), ncol = 2,
          widths = c(0.6, 0.4)
          )


# COMBINE PANELS A & B
# Convert the grob into a patchwork-compatible object
panel_a <- wrap_elements(p_gr)

# Combine them side-by-side
plots <- list(panel_a, panel_b)
...
combined_plot <- panel_a + panel_b +
  plot_layout(widths = c(4, 1)) + # Adjust ratio so Panel A is wider (more panels)
  plot_annotation(tag_levels = 'a') # Automatically labels panels as 'a' and 'b'

# Export as high-resolution for NHB
ggsave("Output/Fig_1ab.png", combined_plot, width = 18, height = 8, units = 'cm', dpi = 300)


ggsave("Output/Fig_1ab.pdf", combined_plot, width = 18, height = 8, units = 'cm', device = cairo_pdf)
