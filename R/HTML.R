...
#'
#' - [`data_MSCA.csv`](https://github.com/MartinBulla/correspondence_funding/blob/main/Data/data_MSCA.csv) contains the score distribution data for the EU *Marie Skłodowska-Curie Actions Postdoctoral Fellowships* for 2018-2025. The data were aggregated from the [EU Funding & Tenders Portal](https://ec.europa.eu/info/funding-tenders/opportunities/portal/), which contains the official "Flash information on the overall results of the call". These reports are published annually by the European Research Executive Agency following the conclusion of the evaluation process. While individual project scores are confidential, these public summaries provide the aggregate percentages of proposals meeting or exceeding specific quality thresholds. The proposals are scored between 0 and 100%.<br>
...
#'  <div style="height: 0.2em;"></div>
#'  **To access the raw reports**: (i) navigate to the EU Funding & Tenders Portal and go to Calls for proposal, (ii) search for the specific call (e.g. HORIZON-MSCA-2024-PF-01-01), (iii) look for the data, usually under the "Topic conditions and documents" or "Updates" section.<br>
#'  <div style="height: 0.2em;"></div>
...
#'
#' - [`data_HFSP.csv`](https://github.com/MartinBulla/correspondence_funding/blob/main/Data/data_HFSP.csv) contains year, rank and score (0-10) of [Research Grant](https://www.hfsp.org/funding/hfsp-funding/research-grants) proposals, written by the applicants invited to write full-blast proposals. Such data are not publicly available, but were kindly provided by [Human Frontiers Science Program](https://www.hfsp.org) for calls with 2024 and 2025 deadlines. We then transformed the scores into percentages (10 representing 100% percent) and to mimic Fig. 1 data, calculated the percentage of evaluated proposals scoring >= each percentage threshold.<br>
#'  <div style="height: 0.2em;"></div>
