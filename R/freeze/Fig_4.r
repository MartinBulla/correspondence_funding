...
dt_long_num = copy(dt_long)

# LOAD data on cut-offs
dt <- fread("Data/data_cut-off.csv")
...
dt_long_cutoff = copy(dt_long)

dt_long = merge(dt_long_num, dt_long_cutoff, by = c("year", "panel", "panel_full"))


# plot & export
p <- ggplot(
  dt_long,
  aes(x = applications, y = cutoff, colour = panel_full)
) +
  #geom_point() +
  geom_line() +
  ...
  labs(
    ...
    y = "Cut-off score for funded proposals [%]",
  ...
  scale_y_continuous(limits = c(88, 98), breaks = seq(88, 98, by = 2), expand = expansion(mult = c(0, 0))) +
  scale_x_continuous(limits = c(0, 4000), expand = expansion(mult = c(0, 0))) +
...
ggsave("Output/Fig_4.png",p, width = 9.5, height = 6, units = 'cm')

p <- ggplot(
  dt_long,
  aes(x = applications, y = cutoff)
) +
  #geom_point() +
  geom_line() +
  #facet_wrap(~ panel_full, nrow = 2) +
...
ggsave("Output/Fig_4b.png",p, width = 9.5, height = 6, units = 'cm')
